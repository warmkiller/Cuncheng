...
if ($article->ID){
	...
	$str .='<!-- Nobird_Seo_Tools End -->' . "\r\n";

}


	$template->SetTags('Nobird_Seo_KeyAndDes',$str); //替换标签  直插最前
...
if ($page==1){$tempurl=$zbp->host;}
  $str .= '<link rel="canonical" href="'.$tempurl.'"/>' . "\r\n";
	...
	$tag=$zbp->GetTagByAliasOrName($zbp->title);
	if ($tag->Intro){
	...
	$zbp->template->SetTags('Nobird_Seo_KeyAndDes',$str); //替换标签  直插最前
