...
if ($zbp->CheckPlugin('UserCenter')) {
		echo '<a href="'.$bloghost.'zb_users/plugin/UserCenter/mng_post.php"><span class="m-left">投稿管理</span></a>';
}
		
		echo '<a href="'.$bloghost.'zb_users/plugin/Nobird_Batch_Articles/help.php"><span class="m-right" style="color:#F00">帮助</span></a>';
		echo '<a href="'.$bloghost.'zb_users/plugin/Nobird_Batch_Articles/setting.php"><span class="m-right" style="color:#F00">插件设置</span></a>';
...
function Nobird_Batch_Admin_ArticleMng(){
	...
	<select class="edit" size="1" name="istop" id="istop" style="width:150px;" ><option value="">置顶任意</option>';
	  echo '<option value="1">置顶</option>';
	  echo '<option value="false">非置顶</option>';
		echo'</select>&nbsp;&nbsp;&nbsp;&nbsp;
	...
	echo'</select>	<input name="search" style="width:250px;" type="text" value="" /> &nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class="button" value="' . $zbp->lang['msg']['submit'] . '"/></p>';
	echo '</form>';
...
$p->PageCount=$zbp->config('Nobird_Batch_Articles')->num; #$zbp->managecount // system default str
...
if(GetVars('istop')){
	$w[]=array('=','log_Istop',GetVars('istop'));
}
if(GetVars('status')){
